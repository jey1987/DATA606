...
1.  In the first paragraph, several key findings are reported. Do these 
    ...
    The findings are sample statistics as it is based on the sample population.
...
2.  The title of the report is "Global Index of Religiosity and Atheism". To
    ...
    There is reasonable representation from each country and all countries are pariticipating.
...
3.  What does each row of Table 6 correspond to? What does each row of 
    ...
    Each row is representing the response from the voter of conducted poll.
...
4.  Using the command below, create a new dataframe called `us12` that contains
    ...
    Yes, the percentage is matching with Table 6
...
p is population count and me is Margin of error.
...
11. If you refer to Table 6, you'll find that Australia has a sample 
    ...
    Yes, because the distribution and the Table 6 data matches
...
Spain – Atheism index is increased , United States – Atheism index has reduced
...
Type 1 error means the Null hypothesis is rejected but it is actually true. Significance level given is 0.05 or 5%. So each country will experience 5% of Type 1 Error. THe number of countries from the table is 40 but for Hong Kong there is no data. So 39 countries will detect 5% change      
...
me = 0.01
n = ( (z/me)^2 )* ( p*(1-p) )
paste("Number of people to sample:",n)
