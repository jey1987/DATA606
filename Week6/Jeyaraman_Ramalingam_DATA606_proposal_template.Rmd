...
india_trade <- read.csv("c:/2018-2010_export.csv") 
india_trade <- filter(india_trade,value>10)
...
india_trade_export_prep <- na_replace(india_trade_export_prep,0)
...
*2. Which countries are top contributors to India's export over time?*
...
ggplot(data=india_trade_export_prep, aes(x=country, y=sum_value, fill=year)) +
  geom_bar( stat="identity") + theme_minimal() + theme(axis.text.x = element_text(angle=90)) + 
  xlab("Countries") + ylab("Amount Exported") + ggtitle("Amount Exported by Countries by Year (Top 100)") 
```
