...
3.  Using `plot_ss`, choose a line that does a good job of minimizing the sum of
    ...
    The smallest number I have got is 185559.9. Depends the points the numbers vary.
...
```{r pred, eval=TRUE}
p_runs = -2789.2429 + 0.6305 * 5578

a_runs = mlb11$runs[mlb11$at_bats==5579] 

res_runs = a_runs - p_runs
```
Since residual is negative , the manager predicted runs is an overestimate
...
6.  Is there any apparent pattern in the residuals plot? What does this indicate
    ...
    The patterns tells us that the there is a linear relationship between runs and at bats.
...
7.  Based on the histogram and the normal probability plot, does the nearly 
    ...
    Yes , The nearly normal residual conditions are met.
...
8.  Based on the plot in (1), does the constant variability condition appear to 
    ...
    The runs and at_bats distribution is constant across the values and hence the condition is met.
...
```{r pred-runs,eval=TRUE}
plot(mlb11$runs ~ mlb11$hits)
m1_hits_runs <- lm(runs ~ hits, data = mlb11)
abline(m1_hits_runs)
```

Yes, There is a linear relationship between hits and runs.
...
```{r pred-runs-atbats,eval=TRUE}
cor(mlb11$runs, mlb11$at_bats)^2
cor(mlb11$runs, mlb11$hits)^2
```    

Yes, Hits predicted at 64% comparing to at_bats at 37%.
...
-   Now that you can summarize the linear relationship between two variables, 
    ...
    five).
```{r best-pred,eval=TRUE}
plot(mlb11$runs ~ mlb11$bat_avg)
cor(mlb11$runs, mlb11$bat_avg)^2
```    

Bat_Avg predicted at 65.6%
...
All three new variables predicted runs better than the other variables.

-   Check the model diagnostics for the regression model with the variable you 
    decided was the best predictor for runs.

```{r best-pred-rmodel,eval=TRUE}

plot(mlb11$runs ~ mlb11$new_obs)
m1_new_obs_runs <- lm(runs ~ new_obs, data = mlb11)
abline(m1_new_obs_runs)
summary(m1_new_obs_runs)
```

The best predictor is new_obs and the rate is 93%
