...
The typical size is 1529.7 and typical means central point of distribution
...
It might not be similar as the sample another student took could vary and hence the typical size.
...
The confidence interval captures the true value with 95% accuracy. Neighbours interval is dependent on teh sample he took for calculating the confidence interval. It should approximately match with the value we calculated.
...
95% of intervals would capture true population mean. Since we have used 1.96 to calculate the interval it is 95% accurate mean. The other sample should also report similar results as the data is normally distributed.
...
-   Using the following function (which was downloaded with the data set), 
    ...
    49 out of 50 are having true population mean and one does not have the true mean. No it is not exactly equal to the confidence level 95%. The confidence levels in this sample is 98%. The actual confidence interval doesnt match with the estimated confidence level. 
...
-   Pick a confidence level of your choosing, provided it is not 95%. What is 
    ...
    The appropriate critical value is 2.33
...
There are three out of 50 are not providing true mean which is only 94% confidence interval. 
   
```{r plot-98c, eval=TRUE}
samp_mean <- rep(NA, 50)
samp_sd <- rep(NA, 50)
n <- 50
