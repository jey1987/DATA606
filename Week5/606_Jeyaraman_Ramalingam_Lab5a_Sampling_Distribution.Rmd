...
The population distribution is skewed left. This means majority of sales are completed for the living area around 1499 sq.ft or lesser. People did not prefer to purchase a property over 1499 sq.ft. of living area
...
4.  How many elements are there in `sample_means50`?  Describe the sampling 
    ...
    In sample_means50 there are 5000 elements as we created an empty vector with 5000 elements. The value for each 50 random sampling is stored in each element. The sampling distribution will be symmetric as the distribution is laid out for the mean value of the 50 samples. The center will be always lie around 1499 which is the population mean. We cannot collect 50000 samples as the data has only 2930 observations.
...
The center will be shrinking towards the population mean 1499 as the sample size increases and the spread will also be reduced. When the sample size reaches the original population the spread will disappear leaving the center value as 1499.
...
-   Take a random sample of size 50 from `price`. Using this sample, what is 
    ...
    The estimate of population mean is around 192557.6 , bit above or bit below.
    
```{r price_estimate, eval=TRUE}
...
The population mean based on the 5000 mean sample size of 50 is 180000.

```{r price_sample,eval=TRUE}
sample_price_means50 <- rep(NA, 5000)

for(i in 1:5000){
  samp_price <- sample(price, 50)
  sample_price_means50[i] <- mean(samp_price)
}

hist(sample_price_means50)

hist(sample_price_means50, breaks = 25)
```
...
-   Change your sample size from 50 to 150, then compute the sampling 
   ...
   The mean sale price of homes based on sample size of 150 is 178000.
   
```{r price_sample_150,eval=TRUE}
 sample_price_means150 <- rep(NA, 5000)
for(i in 1:5000){
  samp_price <- sample(price, 150)
  sample_price_means150[i] <- mean(samp_price)
}

hist(sample_price_means150)

hist(sample_price_means150, breaks = 25)
```
...
The spread is lesser in 3 as the range is from 160000 to 200000 whereas in the Probelem # 2 the spread is ranging from 140000 to 220000. The smaller spread will provide accurate value.