...
```{r inf-weight-ci, eval=TRUE, tidy=FALSE}
inference(y = nc$weight, est = "mean", type = "ci", null = 0, 
          alternative = "twosided", method = "theoretical")
```
...
```{r inf-weight-ci-90, eval=TRUE, tidy=FALSE}
inference(y = nc$weight, est = "mean", type = "ci", null = 0, 
...
```{r inf-weight-ht, eval=TRUE, tidy=FALSE}
...
```{r cut_off, eval=TRUE, tidy=FALSE}
Cutoff_Young_Mom = sqldf("select min(mage) as Min_Cutoff_Young , max(mage) as Max_Cutoff_Young from nc where mature='younger mom'")
Cutoff_Young_Mom

Cutoff_Mature_Mom = sqldf("select min(mage) as Min_Cutoff_Mature , max(mage) as Max_Cutoff_Mature from nc where mature='mature mom'")
Cutoff_Mature_Mom
```
...
```{r inf-weight-visit, eval=TRUE, tidy=FALSE}
