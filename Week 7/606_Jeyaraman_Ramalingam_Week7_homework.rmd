...
```{r ci_90, include=FALSE}
ci_l = 65
ci_u = 77
n = 25

sample_mean = (ci_l+ci_u)/2
sample_sd_l = ((ci_l-sample_mean)*sqrt(n))/(1.645)
sample_sd_u = ((ci_u-sample_mean)*sqrt(n))/(1.645)
sample_sd = (sample_sd_u + (sample_sd_l * -1) )/2

sample_me = round((1.645*round(sample_sd))/sqrt(n))
```
...
As the Confidence Interval increases the sample size increases

(c) Calculate the minimum required sample size for Luke.

```{r sample_sat_99, include=FALSE}
sd = 250
me = 25
z = 2.502
...
The Difference is not observed as the distribution is normal.

(b) Are the reading and writing scores of each student independent of each other?

Reading and writing scores are dependent.
...
1. Sample Distribution should be normal
2. Sample should be independent
...
Since p value is small we can conclude that the clear evidence is not provided.

(f) What type of error might we have made? Explain what the error means in the context of the application.

We failed to reject the alternative hypothesis.

(g) Based on the results of this hypothesis test, would you expect a confidence interval for the average difference between the reading and writing scores to include 0? Explain your reasoning.
Yes, The reasoning is that the null hypothesis is rejected.
...
```{r n_Email, include=FALSE}
SE=2.2
z=0.84
t=2.8
p=0.5

n = round(((t^2)/(p^2))*sqrt((SE^2)+(SE^2)))
n
```
...
H0 - > The average number of hours worked are same
H1 - > The average number of hours worked are not same

(b) Check conditions and describe any assumptions you must make to proceed with the test.

1. The data is independent across the groups
...
SSB = n1 * ((x1-x)^2) + n2 * ((x2-x)^2) + n3 * ((x3-x)^2) + n4 * ((x4-x)^2) + n5 * ((x5-x)^2)
SSB_Tot = SSB + 267382
